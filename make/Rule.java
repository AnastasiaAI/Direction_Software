...
class Rule {
    ...
    private void checkFinishedDependencies() {
        Iteration<Integer> dependencies = _depends.successors(_vertex);
        while (dependencies.hasNext()) {
            int succ = dependencies.next();
            if (_depends.getLabel(succ).outOfDate()) {
                _depends.getLabel(succ).rebuild();
            }
        }
    }
    ...
    private boolean outOfDate() {
        ...
        while (dependencies.hasNext()) {
            int succ = dependencies.next();
            Integer timeSucc = _depends.getLabel(succ).getTime();
            if (timeSucc == null || getTime() < timeSucc) {
                return true;
            }
        }
        return false;
